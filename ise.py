...
class ERS(object):
    ...
    def _request(self, url, method="get", data=None):
        if self.use_csrf:
            if (
                ...
                resp = self.ise.get(
                    "{0}/config/deploymentinfo/versioninfo".format(
                        self.url_base
                    ),
    ...
    def get_sgt(self, sgt):
        ...
        # If not valid OID, perform regular search
            ...
            if isinstance(sgt, int):
                resp = self.ise.get(
                    "{0}/config/sgt?filter=value.EQ.{1}".format(
                        self.url_base,
                        sgt
                    ),
                    timeout=self.timeout
                ...
                resp = self.ise.get(
                    "{0}/config/sgt?filter=name.EQ.{1}".format(
                        self.url_base,
                        sgt
                    ),
                    timeout=self.timeout
    ...
    def get_sgacl(self, sgacl):
        ...
        # If not valid OID, perform regular search
            ...
            resp = self.ise.get(
                "{0}/config/sgacl?filter=name.EQ.{1}".format(
                    self.url_base,
                    sgacl
                ),
                timeout=self.timeout
    ...
    def get_egressmatrixcells(self, size=20, page=1):
        ...
        return self._get_objects(
            "{0}/config/egressmatrixcell".format(
                self.url_base
            ),
            filter=filter,
            size=size,
    ...
    def update_egressmatrixcell(
        ...
        if not emc:
            ...
            resp = self._request(
                ("{0}/config/egressmatrixcell/" + emc).format(
                    self.url_base
                ),
                method="put",
                data=json.dumps(data),
    ...
    def delete_egressmatrixcell(self, emc):
        ...
        resp = self._request(
            "{0}/config/egressmatrixcell/{1}".format(
                self.url_base,
                emc
            ),
            method="delete",
    ...
    def get_identity_group(self, group):
        ...
        resp = self.ise.get(
            "{0}/config/identitygroup?filter=name.EQ.{1}".format(
                self.url_base,
                group
            ),
            timeout=self.timeout
    ...
    def get_user(self, user_id):
        ...
        resp = self.ise.get(
            "{0}/config/internaluser?filter=name.EQ.{1}".format(
                self.url_base,
                user_id
            ),
            timeout=self.timeout
    ...
    def get_device_groups(self, size=20, page=1):
        ...
        """
        return self._get_groups(
            "{0}/config/networkdevicegroup".format(
                self.url_base
            ),
            size=size,
            page=page
    ...
    def update_device_group(self, device_group_oid, name=None, description=None):
        ...
        resp = self._request(
            "{0}/config/networkdevicegroup/{1}".format(
                self.url_base,
                device_group_oid
            ),
            method="put",
    ...
    def get_device(self, device):
        ...
        resp = self.ise.get(
            "{0}/config/networkdevice?filter=name.EQ.{1}".format(
                self.url_base,
                device
            ),
            timeout=self.timeout
