...
def random_mac(separator_char=":", separator_spacing=2):
    ...
    return f"{separator_char}".join(
        unseparated_mac[i : i + separator_spacing]
        for i in range(0, len(unseparated_mac), separator_spacing)
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate and insert x number of random macaddresses into you test ise"
    )
